...
@media(max-width:600px){.tab-nav{width:100%;flex-wrap:wrap;}.tab{font-size:0.6rem;padding:8px 4px;}}
...
</div><div id='sec-schedule' class='section' style='display:none'><div id='schedule'>
...
<button class="sf-btn fav-filter" onclick="toggleFavFilter(this)">⭐ 我的最愛</button>
<button class="sf-btn" onclick="document.querySelectorAll('details').forEach(d=>d.open=true)" style="margin-left:auto;">📂 展開</button>
<button class="sf-btn" onclick="document.querySelectorAll('details').forEach(d=>d.open=false)">📁 收合</button>
</div>
<div class='ml'>{''.join(mm)}</div>
</div>
</div><div id='sec-knockout' class='section' style='display:none'><div id='bracket'>
<h2>🏆 淘汰賽 · 16強至決賽</h2>
{gen_b()}
</div>

</div><div id='sec-trophy' class='section' style='display:none'><div id='trophy'>
...
</div><div id='sec-accuracy' class='section' style='display:none'><div id='strength'>
<h2>📈 實力分佈（48支球隊）</h2>
<div class='sd'>{sd}</div>
</div></div><div class='ft'>Updated: {today} | Generated by Hanni 🐰</div>
</div>
<script>
...
// Using <details> tag - no JS needed for collapse!
document.querySelectorAll('details summary').forEach(function(s){{
  s.addEventListener('click',function(){{
    var d=s.parentElement;
    // summary click toggles details[open] automatically
  }});
}});
